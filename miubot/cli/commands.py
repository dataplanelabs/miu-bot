...
from miubot import __version__, __logo__
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".miubot" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} miubot[/cyan]")
    console.print(body)
    console.print()
...
@app.command()
def onboard():
    """Initialize miubot configuration and workspace."""
    from miubot.config.loader import get_config_path, load_config, save_config
    from miubot.config.schema import Config
    from miubot.utils.helpers import get_workspace_path
    ...
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.miubot/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]miubot agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/miubot#-chat-apps[/dim]")
...
def _make_provider(config):
    ...
    if not (p and p.api_key) and not model.startswith("bedrock/"):
        ...
        console.print("Set one in ~/.miubot/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from miubot.config.loader import load_config, get_data_dir
    from miubot.bus.queue import MessageBus
    from miubot.agent.loop import AgentLoop
    from miubot.channels.manager import ChannelManager
    from miubot.session.manager import SessionManager
    from miubot.cron.service import CronService
    from miubot.cron.types import CronJob
    from miubot.heartbeat.service import HeartbeatService
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from miubot.bus.events import OutboundMessage
...
def agent(
    ...
    from miubot.config.loader import load_config
    from miubot.bus.queue import MessageBus
    from miubot.agent.loop import AgentLoop
    from loguru import logger
    ...
    if logs:
        logger.enable("miubot")
    else:
        logger.disable("miubot")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".miubot" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # miubot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall miubot")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from miubot.config.loader import get_data_dir
    from miubot.cron.service import CronService
...
def cron_add(
    ...
    from miubot.config.loader import get_data_dir
    from miubot.cron.service import CronService
    from miubot.cron.types import CronSchedule
...
def status():
    ...
    if config_path.exists():
        from miubot.providers.registry import PROVIDERS
